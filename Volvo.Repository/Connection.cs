...
using Volvo.Repository.Enums;

namespace Volto.Repository
{
    public abstract class Connection : IDisposable
    {
        ...
        protected void AbrirConexao(IConfiguration configuration, EChaveAcesso chaveAcesso)
        {
            if (chaveAcesso == EChaveAcesso.command)
                _db = new VolvoContext(configuration);
            else {
                _conn = new VolvoContext(configuration).SqlConn;
                _conn.Open();
            }
        }
